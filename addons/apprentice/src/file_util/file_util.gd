...
##[codeblock]
###划分以 “字母头的后面可能带有下划线和其他字母” 的 png 类型文件的资源组
##FileUtil.groups(file_path, "^([A-Za-z]+)_?([A-Za-z]+)?.*?\\.png$")
##
###划分以 “字母_字符和数字.png” 格式的文件名的 png 类型文件的资源组
##FileUtil.groups(file_path, "(\\w+)_(.*?)\\d?\\.png$")
##[/codeblock]
##[b]以括号内的字符串进行分组，不相同的部分不要加到括号里，否则[/b]
##[br]
##[br]返回的结果为 [code]字典数据[分组方式列表] = 文件名列表[/code] 的数据格式
##[br]
##[br]你可以对扫描的文件进行处理。比如生成 [SpriteFrames] 文件
##[codeblock]
##var dir = "文件目录"
##var map = FileUtil.files_group(dir, "(\\w+)_(.*?)\\d?\\.png$")
##var sprite_frames_dict = {}
##for group in map:
##    var sprite_name : String = group[0]
##    var anim_name : String = group[1]
##    var files : Array = map[group]
##    var sprite_frames := sprite_frames_dict.get(sprite_name) as SpriteFrames
##    if sprite_frames == null:
##        sprite_frames = SpriteFrames.new()
##        sprite_frames_dict[sprite_name] = sprite_frames
##    sprite_frames.add_animation(anim_name)
##    sprite_frames.set_animation_loop(anim_name, false)
##    sprite_frames.set_animation_speed(anim_name, 8)
##    for file:String in files:
##        sprite_frames.add_frame(anim_name, load(dir.path_join(file)))
##
##for sprite_name:String in sprite_frames_dict:
##    var sprite_frames = sprite_frames_dict[sprite_name]
##    var path = dir.path_join(sprite_name + ".tres")
...
##[/codeblock]
static func files_group(path: String, pattern: String) -> Dictionary:
	...
	if dir:
		...
		for file in dir.get_files():
			var result : RegExMatch = regex.search(file)
			if result != null:
				# 匹配
				var group : Array = []
				var s = ""
				for i in range(1, group_count + 1):
					...
					if s != "":
						group.append(s)
				# 记录
				if not map.has(group):
					map[group] = []
				map[group].append(file)
