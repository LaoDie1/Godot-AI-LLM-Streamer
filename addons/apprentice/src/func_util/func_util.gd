...
##[br]示例，控制节点列表移动：
##[codeblock]
##FuncUtil.forexec(node_list, ActorUtil.control_move)
##[/codeblock]
...
static func for_range(begin: int, end: int, method: Callable = Callable()):
...
static func get_closest_path(
	...
	while true:
		...
		if list.is_empty():
			...
			continue
		curr = MathUtil.get_closest_points(to, list)
		visited[curr] = null
		paths.append(curr)
...
# 向前移动
static func move_forward_process(body: CharacterBody2D, direction: Vector2, duration, speed):
	body.velocity.y = 0
	if direction.x == 0:
		direction.x = 1
	var process = Process.create_physics_type(
		func():
