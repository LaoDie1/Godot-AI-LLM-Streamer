...
## 执行完这个阶段时发出这个信号，data 数据为调用 [method execute] 执行的数据
...
func alter_stage_time(stage, time: float, sync_current_time: bool = false):
	if (sync_current_time 
		and _stage_point != -1 
		and get_current_stage() == stage
	):
		# 同步修改到当前时间
		stage_time_left += (time - _last_data[stage])
...
func execute(data: Dictionary):
	if not is_node_ready():
		# 必须等待节点 ready 完成之后执行。
		# 如果以 tree_entered 信号，则会出现第一次执行的时间不正确的问题
		await ready 
	if data.is_empty():
