...
# 当前正在执行的技能ID
var _current_execute_skill_ids := {}
# 技能名称对应的技能节点
...
func get_skill(skill_id) -> TimeLine:
	var skill = _name_to_skill_map.get(skill_id)
	if skill:
		return skill
	else:
...
func is_can_execute(skill_id) -> bool:
	return has_skill(skill_id) and not is_executing(skill_id)
...
func add_skill(skill_id, data: Dictionary = {}):
	...
	self.add_child.call_deferred(skill)
	...
	# 执行结束
	var skill_end : Callable = func(tag, end_state):
...
## 移除技能
func remove_skill(skill_id):
	_name_to_skill_map.erase(skill_id)
	self.removed_skill.emit(skill_id)


## 是否有这个技能
func has_skill(skill_id) -> bool:
	return _name_to_skill_map.has(skill_id)
...
func get_skill_data(skill_id) -> Dictionary:
	if has_skill(skill_id):
		return _name_to_data_map[skill_id]
	return {}
...
##修改技能的数据
func execute(skill_id, additional: Dictionary = {}):
...
## 继续执行技能
func continue_execute(skill_id):
	var skill = get_skill(skill_id)
