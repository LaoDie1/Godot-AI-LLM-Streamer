...
static func get_node_from_pool(node_type) -> Node:
	...
	# 这个类型的节点池
	var node_pool : _NodePool = DataUtil.get_value_or_add(type_to_data, node_type, func(): 
...
## 弹出提示信息
static func popup_prompt(message: String, pos: Vector2 = Vector2()):
	var node : Node = Engine.get_main_loop().current_scene
	if not is_instance_valid(node):
		return
	...
	tween.tween_property(label, "global_position:y", label.position.y - 140, DURATION)
	tween.tween_property(label, "modulate:a", 0, DURATION * 0.3).set_delay(DURATION * 0.7)
